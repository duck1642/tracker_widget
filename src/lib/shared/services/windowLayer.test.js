...
describe("applyLayerMode", () => {
  ...
  it("uses fake widget behavior for desktop mode", async () => {
    ...
    expect(invoke).not.toHaveBeenCalledWith("set_true_widget_mode", expect.anything());
  });

  it("resets widget behavior when returning to normal", async () => {
  ...
  it("switches from widget to top by resetting bottom and enabling top", async () => {
  ...
  it("switches from top to widget by resetting top and enabling bottom", async () => {
